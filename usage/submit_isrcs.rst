...
To submit ISRCs, select :menuselection:`"File --> Submit ISRCs"` or click the "Submit ISRCs" button if you have it included in your action toolbar. Picard will check that the ISRCs are valid, that the same ISRC doesn't appear on more than one track on the release, and that the ISRCs are not already present in the database. If all checks pass, the ISRCs will be submitted to the MusicBrainz database.
