...
If there was no AcoustID found when you ran the scan on your files, you don't need to run :menuselection:`"Generate Fingerprints"` on them before submitting the fingerprints to the AcoustID server because Picard will remember the fingerprints generated when you performed the scan. For Picard the AcoustID is primarily an identification tool. Because the files are considered untagged at the initial *identification* stage (when you performed the scan), you can only :menuselection:`"Submit"` fingerprints once the files have been properly matched to a MusicBrainz recording (in the Album pane). You will also find that after submitting fingerprints Picard will not automatically fetch the newly generated AcoustIDs. This is because the generation can take some time, and the response received from the AcoustID server does not contain newly generated AcoustIDs. However, if you do another scan on the files after submission and the AcoustID server has had sufficient time to process the submitted fingerprints, the AcoustIDs should be available.
